...
class GameStateOverride(GameExecutables):
    ...
    def calculate_sequence_win(self, sequence, base_multiplier, length):
        ...
        # Apply mode-specific RTP adjustment AND mode cost multiplier
        # Books must contain final payout: base × adjustment × cost
        current_mode = self.get_current_betmode()
        mode_name = current_mode.get_name()
        mode_cost = current_mode.get_cost()
        adjustment = getattr(self.config, 'mode_rtp_adjustments', {}).get(mode_name, 1.0)
        multiplier = base_multiplier * adjustment * mode_cost
    ...
    def evaluate_finalwin(self):
        ...
        # Record events for RGS frontend display
        self.record_round_events(best_win_result)

        self.repeat = False

    def record_round_events(self, win_result):
        """Record boxing round events for RGS frontend display."""
        # Get current boss mode
        current_mode = self.get_current_betmode()
        boss_name = current_mode.get_name()

        # Record move sequence event
        moves_event = {
            "type": "moveSequence",
            "moves": self.move_sequence,
            "count": len(self.move_sequence)
        }
        self.book.add_event(moves_event)

        # Record combat result if there was a win
        if win_result:
            combat_event = {
                "type": "combatResult",
                "sequence": win_result["sequence"],
                "sequenceLength": win_result["length"],
                "baseMultiplier": win_result["base_multiplier"],
                "finalMultiplier": win_result["final_multiplier"],
                "damage": win_result["damage"],
                "bossHealth": win_result["boss_health"],
                "playerHealth": win_result["player_health"]
            }
            self.book.add_event(combat_event)

        # Record knockout event if achieved
        if self.knockout_achieved:
            knockout_event = {
                "type": "knockout",
                "boss": boss_name,
                "totalDamage": self.damage_dealt,
                "bonusApplied": True
            }
            self.book.add_event(knockout_event)

        # Record final result
        result_event = {
            "type": "roundResult",
            "win": int(round(self.final_win * 100, 0)),  # Convert to integer cents
            "bossHealth": self.boss_health,
            "playerHealth": self.player_health,
            "knockoutAchieved": self.knockout_achieved,
            "wincapTriggered": self.wincap_triggered
        }
        self.book.add_event(result_event)
